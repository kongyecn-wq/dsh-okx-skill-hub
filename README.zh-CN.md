...
> 行情速览：总下载量 219（OKX Skills Marketplace 官方数据），本仓库把 OKX 官方 `okx-cex-market` 技能原样移植进 DSH 生态，并提供 **纯技能目录** 与 **dsh-plugin 插件** 双通道安装。

## 这是什么

[OKX Agent Skills](https://github.com/okx/agent-skills) 是 OKX 官方的即插即用 AI Agent 技能库，通过单个 `okx` CLI 让任意 LLM Agent 查询行情、管理持仓、运行网格/DCA 机器人，无需自行对接 API。本仓库将这些官方技能适配到 DeepSeek Harness 的技能体系（`SKILL.md` 目录束格式），让 DSH 用户开箱即用。
...
| `okx-cex-bot` | 网格/DCA 机器人 | 🚧 待收录（欢迎 PR） | ✅ 需要 |

## 为什么适配 DeepSeek Harness

DSH 的技能格式与 OKX 官方技能**完全同构**（`<技能名>/SKILL.md` + frontmatter `name`/`description` + `references/` 资源目录），因此**零改动的直接复制**即可被 DSH 发现、加载和执行：

| 项目 | DSH 要求 | OKX 官方技能现状 |
|---|---|---|
| 目录结构 | `<name>/SKILL.md`（单层） | `skills/okx-cex-market/SKILL.md` ✅ |
| frontmatter | `name`（kebab-case）+ `description` | `name: okx-cex-market` + `description` ✅ |
...
> 行情类命令全部只读、无需 API Key。交易类技能（portfolio/trade/bot）需要 `OKX_API_KEY` / `OKX_SECRET_KEY` / `OKX_PASSPHRASE`（或 `okx auth login` OAuth）。
...
- [ ] 挂 `dsh-plugin` / `dsh-skill` topic，投稿 awesome 列表与插件市场
- [ ] CI：官方签名校验 + frontmatter 结构检查
...
- [OKX Agent Skills](https://github.com/okx/agent-skills) · [OKX Skills Marketplace](https://www.okx.com/zh-hans/agent-tradekit/skills/okx-cex-market)
